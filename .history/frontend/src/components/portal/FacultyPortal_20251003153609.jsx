    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">Faculty Workspace</p>
            <h1 className="text-2xl font-semibold text-slate-900 mt-1">
              Hello, {user.name}
            </h1>
            <p className="text-sm text-slate-500 mt-2">
              Track cohort momentum, upcoming deadlines, and recognize top performers.
            </p>
          </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-sm text-slate-500">Live cohort insights</span>
            </div>
            <div className="space-y-4">
              {loading ? (
                ...
                overview.courses.map((course) => (
                  <div key={course.courseId} className="border border-slate-200 rounded-2xl p-4">
                    ...
                    {course.dueNext ? (
                      <p className="mt-3 text-sm text-slate-500">
                        Next due: <span className="text-slate-800 font-medium">{course.dueNext}</span>
                      </p>
                    ) : null}
              ...
              <div className="space-y-4">
                {overview.topPerformers.map((leader, index) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-emerald-600">{leader.coins} coins</span>
                  </div>
                ))}
              </div>
            ) : (
