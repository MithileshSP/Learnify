    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-gradient-to-r from-slate-900 via-purple-900 to-indigo-900 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            <p className="text-sm text-white/60 uppercase tracking-wide">Admin Console</p>
            <h1 className="text-3xl font-bold mt-1">Welcome back, {user.name}</h1>
            <p className="text-white/70 mt-2">
              Monitor campus-wide engagement, quest performance, and student momentum.
            </p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6 col-span-1 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-gray-500">Across all active users</span>
            </div>
            <div className="text-4xl font-bold text-gray-900">
              {loading ? "-" : `${overview?.averageCoins?.toFixed(1) ?? 0} ☀️`}
            </div>
            <p className="text-sm text-gray-500 mt-2">
              Use coin trends to identify cohorts that might need nudges or fresh quests.
            </p>
          </div>
            ...
            <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
              {loading ? (
                ...
                overview.recentActivity.map((entry, index) => (
                  <div key={`${entry.userName}-${index}`} className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold text-gray-800">{entry.userName}</p>
                      <p className="text-sm text-gray-500">{entry.questTitle}</p>
                    </div>
                ...
                <p className="text-gray-500">No recent quest completions logged.</p>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">Sorted by coins earned</span>
          </div>
                ...
                {loading ? (
                  ...
                  overview.leaderboard.map((entry) => (
                    <tr key={entry.id ?? entry.name}>
                      <td className="px-4 py-4 text-gray-900 font-medium">{entry.name}</td>
                      <td className="px-4 py-4 text-gray-600">{entry.completedQuests}</td>
                      <td className="px-4 py-4 text-gray-600">{entry.streak} days</td>
                      <td className="px-4 py-4 text-gray-900 font-semibold">{entry.coins}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
...
function StatCard({ icon: Icon, label, value, accent, loading }) {
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
      <div className={`inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br ${accent} text-white mb-4`}>
