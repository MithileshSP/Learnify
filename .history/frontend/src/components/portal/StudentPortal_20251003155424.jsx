    ...
    <div className="min-h-screen bg-slate-100 flex text-slate-900">
      <aside className="hidden lg:flex w-64 flex-col border-r border-slate-200 bg-white/90 backdrop-blur">
        <div className="px-6 py-8 border-b border-slate-100">
          <div className="text-2xl font-semibold text-indigo-600">LearnOnline</div>
        </div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 xl:col-span-2">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Daily Quests</h2>
              <p className="text-sm text-slate-500">Complete tasks and earn rewards today.</p>
            </div>
        ...
        <div className="px-6 py-6 space-y-4">
          {courses.length === 0 && !loading ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-8 text-center text-slate-500">
              No active courses yet. Enroll in a course to see your progress here.
            </div>
          ) : null}
    ...
    <section className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="rounded-full bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-600">
          {quests.filter((quest) => quest.completed).length} / {quests.length} completed
        </div>
      </div>
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Stay ahead with curated research, campus innovation, and industry insights.
        </p>
      </div>
...
function GradesView({ grades, courses, loading }) {
  return (
    <section className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-slate-900">Grades & Progress</h2>
        <p className="text-sm text-slate-500">
      ...
      {grades.length === 0 && !loading ? (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center text-slate-500">
          Grade records will appear here once your courses start reporting progress.
        </div>
      ) : null}
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
              {grades.map((course) => (
                <tr key={course.courseId} className="hover:bg-slate-50/80">
                  ...
                  <td className="px-6 py-4 font-semibold text-indigo-600">{course.grade}</td>
    ...
    <section className="space-y-8">
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        ...
        <div className="flex flex-col items-start gap-3">
          ...
          <p className="text-xs text-slate-500">
            Tip: You can return anytime with single sign-on using your campus email.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Badges unlock as you complete milestones, maintain streaks, and help your peers.
          </p>
        </div>
      </div>
    </section>
  ...
  return (
    <div
      className={`flex flex-col gap-4 rounded-3xl border p-6 transition md:flex-row md:items-center md:justify-between ${
        quest.completed ? "border-emerald-100 bg-emerald-50" : "border-slate-100 bg-white shadow-sm"
      }`}
    >
      <div className="flex items-start gap-4">
        <div
          className={`mt-1 flex h-10 w-10 items-center justify-center rounded-full ${
            quest.completed ? "bg-emerald-100 text-emerald-600" : "bg-indigo-50 text-indigo-500"
          }`}
        >
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {quest.completed ? "Completed" : acting ? "Updating..." : "Start Quest"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white px-6 py-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <p className="text-sm text-slate-500">Instructor: {course.instructor || "TBA"}</p>
        {course.dueNext ? (
          <p className="mt-2 text-xs text-slate-500">
            Next up: <span className="font-medium text-slate-700">{course.dueNext}</span>
          </p>
        ) : null}
      </div>
