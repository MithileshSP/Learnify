...
export default function FacultyPortal({ user, onLogout }) {
    ...
    return [
      {
        ...
        delta: courseProgress.length
          ? `${courseProgress.length} active cohorts`
          : "",
        icon: BookOpen,
  ...
  <main className="max-w-7xl mx-auto app-content debug-outline space-y-10">
        {error ? (
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
          {summaryCards.map((card) => (
            ...
            >
              <div
                className={`w-10 h-10 rounded-2xl bg-gradient-to-r ${card.accent} text-white flex items-center justify-center`}
              >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {overview?.aiGrading?.pendingCount ?? 0} items waiting for
                    your judgement
                  </p>
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <header className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                        ...
                        <span
                          className={`inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold rounded-full ${statusClass}`}
                        >
                      ...
                      <footer className="mt-5 flex flex-wrap items-center gap-3">
                        <button
                          type="button"
                          onClick={() =>
                            handleReviewSuggestion(suggestion.id, "approved")
                          }
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleReviewSuggestion(suggestion.id, "reviewed")
                          }
                          disabled={isBusy("suggestion", suggestion.id)}
            ...
            <header className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {overview?.mentorship?.activeCount ?? 0} active mentees •
                    Last sync {overview?.mentorship?.lastUpdated || "Recently"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                overview.courses.map((course) => {
                  const toneClass =
                    courseToneStyles[course.statusTone] ||
                    courseToneStyles.indigo;
                  return (
                    <div
                      key={course.id}
                      className="border border-slate-200 rounded-2xl p-4 bg-slate-50 flex flex-col gap-3"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Code {course.code || "—"} • Updated{" "}
                            {formatDateTime(course.lastUpdated)}
                          </p>
                        </div>
                        <span
                          className={`inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold rounded-full ${toneClass}`}
                        >
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Live cohort insights across your classes
                  </p>
                </div>
              </div>
            </div>
            <div className="space-y-4">
              {loading ? (
                ...
                <p className="text-sm text-slate-500">
                  No cohort data yet. Once students enroll, momentum will appear
                  here.
                </p>
              ) : (
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <p className="text-sm text-slate-500">
                  Encourage students to complete quests to populate the
                  leaderboard.
                </p>
              ) : (
