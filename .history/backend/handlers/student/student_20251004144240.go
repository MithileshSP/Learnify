...
func GetStudentDashboard(w http.ResponseWriter, r *http.Request) {
	...
	researchFeed, err := collectResearchFeed(ctx, targetID, 25); if err != nil { common.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()}); return }
...
// Research feed subset reused (trimmed copy to keep student package independent of yet-to-be-split research code)
func collectResearchFeed(ctx context.Context, viewerID int, limit int64) ([]models.ResearchPostResponse, error) {
	...
	cursor, err := common.ResearchPostsCol.Find(ctx, bson.M{}, findOpts); if err != nil { return nil, fmt.Errorf("failed to load research posts: %w", err) }; defer cursor.Close(ctx)
...
func buildResearchPostResponse(post models.ResearchPost, viewerID int) models.ResearchPostResponse {
	...
	createdAt := ""; if !post.CreatedAt.IsZero() { createdAt = post.CreatedAt.UTC().Format(timeLayout()) }
...
// --- simplified copies of original helpers (kept local to avoid deep coupling) ---
func isResearchPostTrending(post models.ResearchPost) bool { score := post.Likes + post.Comments + (post.Collaborations*3); if score >= 30 { return true }; if timeSince(post.CreatedAt) <= (72*timeHour()) && score >= 12 { return true }; return false }
func formatRelativeTimestamp(t timeType) string { if t.IsZero() { return "Recently" }; diff := timeSince(t); if diff < timeMinute() { return "Just now" }; if diff < timeHour() { m := int(diff/timeMinute()); if m<=1 { return "1 minute ago" }; return fmt.Sprintf("%d minutes ago", m) }; if diff < 24*timeHour() { h := int(diff/timeHour()); if h<=1 { return "1 hour ago" }; return fmt.Sprintf("%d hours ago", h) }; if diff < 7*24*timeHour() { d := int(diff/(24*timeHour())); if d<=1 { return "1 day ago" }; return fmt.Sprintf("%d days ago", d) }; if diff < 30*24*timeHour() { w := int(diff/ (7*24*timeHour())); if w<=1 { return "1 week ago" }; return fmt.Sprintf("%d weeks ago", w) }; return t.Format("Jan 2, 2006") }
func normalizeResearchCategory(raw string, isCollab bool) string { c := strings.TrimSpace(raw); if c=="" { if isCollab { return "Collaboration" }; return "My Research" }; switch strings.ToLower(c) { case "collaboration","collaborations","collab","team-up": return "Collaboration"; case "my research","research","personal": return "My Research"; case "trending": return "Trending"; default: return c } }

// --- tiny time abstraction to keep this file self-contained ---
// We purposely alias to allow replacement/testing later without importing full time everywhere.
type timeType = struct{ time.Time }

// Since we can't define methods on time.Time alias easily in this context, use direct time package wrappers.
// For simplicity replace with direct time usage:
import "time"
...
func timeLayout() string { return time.RFC3339 }
