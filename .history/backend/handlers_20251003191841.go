...
func aiChat(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Map known API errors to their HTTP status codes when possible
		var apiErr *geminiAPIError
		if errors.As(err, &apiErr) {
			// Use 429 for rate-limits, 404 for missing models, else 502 as a gateway error
			status := http.StatusBadGateway
			if apiErr.StatusCode == http.StatusTooManyRequests {
				status = http.StatusTooManyRequests
			} else if apiErr.StatusCode == http.StatusNotFound {
				status = http.StatusBadGateway // hide 404 specifics but keep as upstream issue
			}
			payload := map[string]interface{}{"error": err.Error()}
			if apiErr.StatusCode == http.StatusTooManyRequests && apiErr.RetryAfterSeconds > 0 {
				payload["retryAfterSeconds"] = apiErr.RetryAfterSeconds
			}
...
func callGeminiAPI(apiKey, message string) (string, error) {
	...
	if lastErr != nil {
		// Try model discovery as a final fallback
		discovered, derr := discoverGeminiModels(apiKey)
		if derr == nil && len(discovered) > 0 {
			for _, m := range discovered {
				resp, err := invokeGeminiModel(apiKey, message, m)
				if err == nil {
					return resp, nil
				}
				if !isModelFallbackError(err) {
...
func invokeGeminiModelVersion(apiKey, message, model, version string) (string, error) {
	...
	if resp.StatusCode >= 400 {
		...
		return "", parseGeminiError(version, resp.StatusCode, body, retryAfter)
	}
...
func parseGeminiError(version string, statusCode int, body []byte, retryAfterSeconds int) error {
	var apiErr struct {
		Error struct {
			Code    int    `json:"code"`
			Message string `json:"message"`
	...
	if err := json.Unmarshal(body, &apiErr); err == nil && (apiErr.Error.Code != 0 || apiErr.Error.Message != "") {
		// Normalize known quota messages for clarity
...
func isModelFallbackError(err error) bool {
	...
	// Treat certain non-API errors as retryable fallbacks
	msg := strings.ToLower(err.Error())
	if strings.Contains(msg, "empty response") {
		return true
	}
	if strings.Contains(msg, "refused to answer") {
		return true
	}
	if strings.Contains(msg, "blocked the prompt") {
		return true
	}
	return false
}
...
// parseRetryAfter parses either seconds or HTTP-date formats; returns seconds to wait
...
func discoverGeminiModels(apiKey string) ([]string, error) {
	...
	for _, ver := range versions {
		url := fmt.Sprintf("https://generativelanguage.googleapis.com/%s/models?key=%s", ver, apiKey)
		...
		// Filter for generateContent-capable and prefer flash family
		var names []string
		for _, m := range parsed.Models {
			...
			if strings.Contains(name, "gemini-1.5-flash") {
				names = append(names, name)
			}
		}
		if len(names) == 0 {
			// fall back to any generateContent model if no flash found
			for _, m := range parsed.Models {
				hasGen := false
				for _, method := range m.SupportedGenerationMethods {
					if strings.EqualFold(method, "generateContent") {
		...
		// Rank: latest first, then shorter names
		sort.SliceStable(names, func(i, j int) bool {
			ai, aj := names[i], names[j]
			li := strings.Contains(ai, "latest")
			lj := strings.Contains(aj, "latest")
			if li != lj {
				return li && !lj
			}
	...
	return nil, fmt.Errorf("could not list models")
}
