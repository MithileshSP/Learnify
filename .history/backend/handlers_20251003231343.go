...
func addFacultyMentee(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, mongo.ErrNoDocuments) {
		...
		if _, insertErr := facultyDashboardsCol.InsertOne(ctx, newDoc); insertErr != nil {
...
func updateFacultyMenteeStatus(w http.ResponseWriter, r *http.Request) {
	...
	// Apply mentee field updates separately to avoid losing nested updates.
	_, updateErr := facultyDashboardsCol.UpdateOne(
		ctx,
		bson.M{"faculty_id": targetID, "mentorship.mentees._id": menteeID},
		bson.M{
			"$set": setFields,
	...
	// Reload updated document for response
	if err := facultyDashboardsCol.FindOne(ctx, bson.M{"faculty_id": targetID}).Decode(&doc); err != nil {
